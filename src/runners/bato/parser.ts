...
export class Parser {
  getHomepageSection(
    ...
    const selector = id === "popular" ? "div.home-popular div.col.item" : "div.series-list div.col.item"
    const items = $(selector).toArray();
    return items.map((element) => this.parseHighlight($, element));
  }

  isLastPage($: CheerioAPI) {
    return !!$(".page-item.disabled > span:contains('»')").length;
  }
  ...
  parseContent($: CheerioAPI, contentId: string): Content {
  ...
  parsePages($:CheerioAPI): ChapterPage[] {
